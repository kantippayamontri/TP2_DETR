...
class PostProcess(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, target_sizes, eval_proposal=False, calibrate=False, calibrate_type='sigmoid'):
        ...
        if self.enable_freqCalibrate and calibrate:
            if calibrate_type=='sigmoid':
                fft_magnitudes = outputs['fft_magnitudes']
                fft_magnitudes = fft_magnitudes.sigmoid()
                prob = torch.mul(prob,fft_magnitudes)
            elif calibrate_type=='sigmoid_enlarge':
                fft_magnitudes = outputs['fft_magnitudes']
                fft_magnitudes = 2*(fft_magnitudes.sigmoid() - 0.5)
                prob = torch.mul(prob,fft_magnitudes)
            # fft_magnitudes = fft_magnitudes.sigmoid()
            # prob = torch.mul(prob,fft_magnitudes).softmax(-1)
            # fft_magnitudes = torch.tanh(fft_magnitudes)
            # prob = torch.mul(prob,fft_magnitudes).softmax(-1)
