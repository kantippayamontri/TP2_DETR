    ...
    def forward(self, query_feat, video_feat, roi_segment_feat,
        ...
        if self.refine_fusion_type == "ca":
            ...
            if self.enable_refine_freq:
                Z = torch.fft.fft(roi_segment_feat.cpu(), dim=2).to(query_feat.device)  # 沿時間維度做 FFT
                segment_feat_freq = torch.abs(Z)**2
                segment_feat_freq = segment_feat_freq.permute(2,1,0,3).reshape(l,n*b,dim) # [l,n*b,dim]

                tgt1 = self.cross_attn_local(query=query_feat_seg,
                                            key=segment_feat,
                                            value=segment_feat_freq)[0] # [1,n*b,dim]
            else:
                tgt1 = self.cross_attn_local(query=query_feat_seg,
                                            key=segment_feat,
