...
class DeformableDETR(nn.Module):
    def __init__(self, 
        ...
        self.dino_two_stage = args.dino_two_stage
        ...
        if self.separate_predict_head: 
            ...
            num_pred = (transformer.decoder.num_layers + 1) if (self.two_stage or self.dino_two_stage) else transformer.decoder.num_layers
            ...
            if self.dino_two_stage:
                self.transformer.decoder.class_embed = self.actionness_embed
                self.transformer.decoder.bbox_embed = self.bbox_embed
            if self.two_stage:
    ...
    def forward(self, samples: NestedTensor, classes_name, description_dict, targets, epoch, pretrain=False, seen_classes_names=None, unseen_classes_names=None, batch_idx=-1):
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, salient_preparation, sparse_preparation, sa_attn_weights_dec, self_attn_weights_dec \
            = self.transformer(srcs, masks, pos, query_embeds, weights=self.salient_weights, semantic_descriptions=semantic_descriptions, pretrain=pretrain, epoch=epoch)
        ...
        if self.enable_refine or self.enable_refine_freq:
            ...
            for lvl in range(hs.shape[0]):
                ...
                if self.separate_predict_head:
                    ...
                    tmp = self.bbox_embed[lvl](hs[lvl])                                     # [b,num_queries,2]
                else:
                    if self.target_type != "none":
