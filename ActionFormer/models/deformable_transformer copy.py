...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=512, nhead=8,
                 ...
                 two_stage=False, two_stage_num_proposals=40, 
                 use_SAPM=False, use_SAattn=False, use_VideoSA=False, SA_LastOnly=False, 
                 dino_two_stage=False, num_queries=40):
        super().__init__()
        ...
        self.dino_two_stage = dino_two_stage
        ...
        self.decoder = DeformableTransformerDecoder(decoder_layer, sa_decoder_layer, num_decoder_layers, return_intermediate_dec, usage_SAattn, SA_LastOnly)
        ...
        # if two_stage or dino_two_stage:
    ...
    def _reset_parameters(self):
        ...
        for m in self.modules():
            if isinstance(m, MSDeformAttn):
                m._reset_parameters()
        if not (self.two_stage or self.dino_two_stage):
    ...
    def gen_encoder_output_proposals_dino(self, memory, memory_padding_mask, temporal_shapes):
        N_, S_, C_ = memory.shape
        base_scale = 4.0
        proposals = []
        _cur = 0
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, weights=None, semantic_descriptions=None, pretrain=False, epoch=-1):
        if epoch<5:
        ...
        if self.two_stage:
            output_memory, output_proposals = self.gen_encoder_output_proposals(memory, mask_flatten, temporal_shapes)      # [b, sum(t), c], 

            # hack implementation for two-stage Deformable DETR
            enc_outputs_class = self.decoder.class_embed[self.decoder.num_layers](output_memory)                            # [b, sum(t), 1] *need to modify class_embed
            enc_outputs_coord_unact = self.decoder.bbox_embed[self.decoder.num_layers](output_memory) + output_proposals    # [b, sum(t), 2]

            topk = self.two_stage_num_proposals
            topk_proposals = torch.topk(enc_outputs_class[..., 0], topk, dim=1)[1]                                          # [b, num_queries] *enc_outputs_class[..., 0]似乎代表"foreground objectness"
            topk_coords_unact = torch.gather(enc_outputs_coord_unact, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 2))      # [b, num_queries, 2] unsigmoid 有通過head
            topk_coords_unact = topk_coords_unact.detach()                                                                  # [b, num_queries, 2] 
            reference_points = topk_coords_unact.sigmoid()                                                                  # [b, num_queries, 2]
            init_reference_out = reference_points                                                                           # [b, num_queries, 2]
            pos_trans_out = self.pos_trans_norm(self.pos_trans(self.get_proposal_pos_embed(topk_coords_unact)))             # [b, num_queries, c]
            if self.use_SAPM:
                query_embed, _ = torch.split(pos_trans_out, c, dim=2) # 只取 positional query
            else:
                query_embed, tgt = torch.split(pos_trans_out, c, dim=2)
        elif self.dino_two_stage:
            output_memory, output_proposals = self.gen_encoder_output_proposals_dino(memory, mask_flatten, temporal_shapes)     # [b, sum(t), c], [b, sum(t), 2]
            output_memory = self.enc_output_norm(self.enc_output(output_memory))                                                # [b, sum(t), c]
            
            enc_outputs_class_unselected = self.decoder.class_embed[self.decoder.num_layers](output_memory)                     # [b, sum(t), 1]
            enc_outputs_coord_unselected = self.decoder.bbox_embed[self.decoder.num_layers](output_memory) + output_proposals   # [bs, sum(t), 2] unsigmoid
            topk = self.num_queries
            topk_proposals = torch.topk(enc_outputs_class_unselected[..., 0], topk, dim=1)[1]                                   # [b, num_queries]

            refpoint_embed_undetach = torch.gather(enc_outputs_coord_unselected, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 2))   # [b, num_queries, 2] unsigmoid 有通過head
            refpoint_embed_ = refpoint_embed_undetach.detach()                                                                      # [b, num_queries, 2] 
            init_box_proposal = torch.gather(output_proposals, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 2)).sigmoid()           # [b, num_queries, 2] sigmoid 沒通過head
            
            tgt = self.tgt_embed.weight[:, None, :].repeat(1, bs, 1).transpose(0, 1)    # [b, num_queries, c]
            reference_points = refpoint_embed_.sigmoid()                                # [b, num_queries, 2]
        ...
        # End preparing tgt
        # - tgt: [b, num_queries, c]
        # - refpoint_embed(unsigmoid): [b, num_queries, c]
        #########################################################
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, sa_decoder_layer, num_layers, return_intermediate=False, usage_SAattn=False, SA_LastOnly=False):
        ...
        #     input_dim=256,    # = 2 * num_pos_feats
        #     hidden_dim=512,   # = d_model
        #     output_dim=512,   # = d_model
        #     num_layers=2      # 兩層 MLP，夠用
        # )
    ...
    def forward(self, tgt, reference_points, src, src_temporal_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            # query_pos = self.ref_point_head(gen_sineembed_for_position(reference_points_input[:, :, 0, :])) 
...
def gen_sineembed_for_position(pos_tensor):
    """
    Args:
        pos_tensor: Tensor of shape [B, N, D], where D = 1 (center) or 2 (center, length)
        * num_pos_feats = 128
    Returns:
        pos: Tensor of shape [B, N, 256] — sine positional embedding
    """
    scale = 2 * math.pi
    dim_t = torch.arange(128, dtype=torch.float32, device=pos_tensor.device)
    dim_t = 10000 ** (2 * (dim_t // 2) / 128)

    # Case 1: only center is given (e.g., pos_tensor.shape[-1] == 1)
    if pos_tensor.size(-1) == 1:
        t_embed = pos_tensor[:, :, 0] * scale  # [B, N]
        pos_t = t_embed[:, :, None] / dim_t    # [B, N, 128]
        pos_t = torch.stack((pos_t[:, :, 0::2].sin(), pos_t[:, :, 1::2].cos()), dim=3).flatten(2)  # [B, N, 128]
        pos = pos_t

    # Case 2: center and length are given (e.g., pos_tensor.shape[-1] == 2)
    elif pos_tensor.size(-1) == 2:
        t_embed = pos_tensor[:, :, 0] * scale  # center
        l_embed = pos_tensor[:, :, 1] * scale  # length

        pos_t = t_embed[:, :, None] / dim_t
        pos_l = l_embed[:, :, None] / dim_t

        pos_t = torch.stack((pos_t[:, :, 0::2].sin(), pos_t[:, :, 1::2].cos()), dim=3).flatten(2)  # [B, N, 128]
        pos_l = torch.stack((pos_l[:, :, 0::2].sin(), pos_l[:, :, 1::2].cos()), dim=3).flatten(2)  # [B, N, 128]

        pos = torch.cat((pos_t, pos_l), dim=2)  # [B, N, 256]

    else:
        raise ValueError(f"Unsupported pos_tensor.shape[-1]: {pos_tensor.shape[-1]} (expected 1 or 2)")
...
def build_deforamble_transformer(args):
    return DeformableTransformer(
        ...
        dino_two_stage = args.dino_two_stage, 
