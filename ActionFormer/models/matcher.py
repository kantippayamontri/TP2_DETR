...
class HungarianMatcher(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, chosen_logits_type):
        """ Performs the matching
        Params:
            outputs: This is a dict that contains at least these entries:
                 "pred_logits": Tensor of dim [batch_size, num_queries, num_classes] with the classification logits (chosen_logits_type = "actionness_logits" or "class_logits")
        ...
        Returns:
            ...
            For each batch element, it holds:
                len(index_i) = len(index_j) = min(num_queries, num_target_boxes) <-- Usually assume num_queries>>num_target_boxes, so len(index_i) = len(index_j) = num_target_boxes
        """
        with torch.no_grad():
            bs, num_queries = outputs[chosen_logits_type].shape[:2] 

            # We flatten to compute the cost matrices in a batch
            out_prob = outputs[chosen_logits_type].flatten(0, 1).sigmoid() # [batch_size * num_queries, 1]
            ...
            # Final cost matrix
            C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * cost_giou # lambda* L_cls + (alpha* L_1 + beta*L_tIoU)
            C = C.view(bs, num_queries, -1).cpu()

            sizes = [len(v["segments"]) for v in targets]
            indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))] # linear_sum_assignment: Bipartite Matching
            return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices]
 


def build_matcher(args):
    return HungarianMatcher(cost_class=args.set_cost_class, # default: 2(lambda)
                            cost_bbox=args.set_cost_bbox,   # default: 5(alpha)
                            cost_giou=args.set_cost_giou,   # default: 2(beta)
                            args = args
                            )