...
from utils.dam import attn_map_to_flat_grid, attn_map_to_ac_last_layer, compute_G_D, attn_map_to_ac_multi_layer, attn_map_to_ac_multi_layer_vec, attn_map_to_flat_grid_per_query
from utils.draw import plot_attention_map, plot_SAattention_map, visualize_gd, visualize_selfattn
...
class DeformableDETR(nn.Module):
    def __init__(self, 
        ...
        self.use_VideoSA = args.use_VideoSA
    ...
    def forward(self, samples: NestedTensor, classes_name, description_dict, targets, epoch, pretrain=False, seen_classes_names=None, unseen_classes_names=None, batch_idx=-1):
        ...
        if self.target_type != "none":
            with torch.no_grad():
                if self.feature_type == "ViFi-CLIP":
                    ...
                    all_text_feats = self.get_text_feats(seen_classes_names + unseen_classes_names, description_dict, self.device, self.target_type) # [N classes,dim]
                else:
                    raise NotImplementedError
        ...
        sa_attn_weights_dec = [num_decoder_layers, b, num_quries, num_classes(seen+unseen)]
        '''
        semantic_descriptions = None
        if self.use_SAattn:
            semantic_descriptions = text_feats
        elif self.use_VideoSA:
            semantic_descriptions = all_text_feats
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, salient_preparation, sparse_preparation, sa_attn_weights_dec, self_attn_weights_dec \
            = self.transformer(srcs, masks, pos, query_embeds, weights=self.salient_weights, semantic_descriptions=semantic_descriptions, pretrain=pretrain)
        ...
        # plot_SAattention_map(sa_attn_weights_dec[-1])
        # flat_grid_attn_map_dec = attn_map_to_flat_grid(
        #     temporal_shapes, level_start_index, sampling_locations_dec.transpose(0, 1), attn_weights_dec.transpose(0, 1)).sum(dim=(1,2))
        # plot_attention_map(flat_grid_attn_map_dec, temporal_shapes, level_start_index)
        # out['attn_map'] = flat_grid_attn_map_dec

        if self.feedback_loss:
            A_C_all = attn_map_to_flat_grid_per_query(temporal_shapes, level_start_index, sampling_locations_dec, attn_weights_dec)
            out['G_D'] = compute_G_D(A_C_all)
            out['A_D'] = self_attn_weights_dec
            # print("A_C max:", A_C_all.max().item(), "min:", A_C_all.min().item())
            if batch_idx==0:
                visualize_gd(A_C_all[-1], epoch=epoch, training_mode=True)
                visualize_selfattn(self_attn_weights_dec[-1], epoch=epoch, training_mode=True)
        ...
        if self.salient_loss:
            ...
            # # ------------------------------------------------------------------------------------------------------------------
            # # compute salient_logits for different scales
            # fpn_salient_logits_list = []
            # for lvl in range(self.num_feature_levels):
            #     if lvl == self.num_feature_levels-1:
            #         fpn_salient_logits = self.salient_heads[lvl](srcs[lvl]).permute(0,2,1)                    
            #     else:
            #         fpn_salient_logits = self.salient_heads[lvl](srcs[lvl]).permute(0,2,1)
            #     fpn_salient_logits_list.append(fpn_salient_logits)
            # # upsample for different scales back to [b, t, 1]
            # if self.salient_upsample_type == 'nearest':
            #     fpn_salient_logits_upsampled = [
            #         salient if lvl < self.enc_stem_layers else F.interpolate(salient.transpose(1, 2), size=(t,), mode='nearest').transpose(1, 2)
            #         for lvl, salient in enumerate(fpn_salient_logits_list)
            #     ]
            # else:
            #     fpn_salient_logits_upsampled = [
            #         salient if lvl < self.enc_stem_layers else F.interpolate(salient.transpose(1, 2), size=(t,), mode='linear', align_corners=False).transpose(1, 2)
            ...
            # # aggregate for different scales
            # if self.salient_aggregate_type == 'max':
            #     fpn_salient_logits = torch.max(torch.stack(fpn_salient_logits_upsampled, dim=0), dim=0)[0]
            # elif self.salient_aggregate_type == 'mean':
            #     fpn_salient_logits = torch.mean(torch.stack(fpn_salient_logits_upsampled, dim=0), dim=0)
            # elif self.salient_aggregate_type == 'mix':
            #     fpn_salient_logits = 0.5 * torch.mean(torch.stack(fpn_salient_logits_upsampled, dim=0), dim=0) + \
            #                      0.5 * torch.max(torch.stack(fpn_salient_logits_upsampled, dim=0), dim=0)[0]
            # else:
            #     # w_normalized = torch.softmax(self.salient_weights, dim=0)
            #     w_normalized = self.salient_weights / self.salient_weights.sum()
            #     fpn_salient_logits = sum(w_normalized[i] * fpn_salient_logits_upsampled[i] for i in range(self.num_feature_levels))
            ...
            multi_scale_feats = []
            for lvl in range(self.num_feature_levels):
                feat = srcs[lvl].permute(0, 2, 1) # [b, t_i, c]
            ...
            # 4. pass through MLP to get salient_logits: [b, t, 1]
            fpn_salient_logits = self.salient_embed(fused_feat)
            out['fpn_salient_logits'] = fpn_salient_logits
        ...
        if self.enable_refine or self.enable_refine_freq:
            ...
            for lvl in range(hs.shape[0]):
                ...
                if self.separate_predict_head:
                    if self.target_type != "none":
                        class_emb = self.class_embed[lvl](hs[lvl])                          # [b, num_queries, num_classes]
                        outputs_class = self._compute_similarity(class_emb, text_feats)     # [b, num_queries, num_classes]
                    else:
                        outputs_class = self.class_embed[lvl](hs[lvl])                      # [b, num_queries, num_classes]
                    outputs_actionness = self.actionness_embed[lvl](hs[lvl])                # [b,num_queries,1]
                    tmp = self.bbox_embed[lvl](hs[lvl])                                     # [b,num_queries,2] (Δcenter, Δlength)
                else:
                    if self.target_type != "none":
                        class_emb = self.class_embed(hs[lvl])                               # [b, num_queries, num_classes]
                        outputs_class = self._compute_similarity(class_emb, text_feats)     # [b, num_queries, num_classes]
                    else:
                        outputs_class = self.class_embed(hs[lvl])                           # [b, num_queries, num_classes]
                    outputs_actionness = self.actionness_embed(hs[lvl])                     # [b,num_queries,1]
                    tmp = self.bbox_embed(hs[lvl])                                          # [b,num_queries,2]
                # ---------------------------------------------------------------------------------------------------------
                if reference.shape[-1] == 2:
                    tmp += reference
                else:
        ...
        if self.semantic_guided_loss:
            out['semantic_guided_logits'] = sa_attn_weights_dec[-1]
        ...
        if self.aux_loss:
            ...
            if self.semantic_guided_loss and not self.SA_LastOnly:
                for i in range(len(out['aux_outputs'])):
                    out['aux_outputs'][i]['semantic_guided_logits'] = sa_attn_weights_dec[i]
...
def build(args, device):
    ...
    if args.semantic_guided_loss:
        weight_dict['loss_semantic_guided'] = args.semantic_guided_loss_coef
    if args.vis_con_loss:
        weight_dict['loss_visual_consistency'] = args.vis_con_loss_coef
    if args.sparsity_loss:
        weight_dict['loss_sparsity'] = args.sparsity_loss_coef
    if args.feedback_loss:
        weight_dict['loss_feedback'] = args.feedback_loss_coef
    ...
    if args.semantic_guided_loss and not args.SA_LastOnly:
        losses = ['boxes','actionness', 'semantic_guided'] # new
    elif args.eval_proposal or args.enable_classAgnostic:
        losses = ['boxes','actionness'] # default
    elif args.actionness_loss:
        losses = ['labels','actionness','boxes']
    else:
        losses = ['labels', 'boxes']
    criterion = build_criterion(args, num_classes, matcher=matcher, weight_dict=weight_dict, losses=losses, focal_alpha=args.focal_alpha, split_id=args.split_id)
    criterion.to(device)
