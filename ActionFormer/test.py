...
def test(model, 
    ...
    for samples, targets in data_loader:
        ...
        outputs = model(samples, classes, description_dict,targets,epoch, seen_classes_names=seen_classes_names, unseen_classes_names=unseen_classes_names)
        ...
        results = postprocessor(outputs, video_duration, args.eval_proposal, calibrate=False)
    ...
    if action_evaluator is not None:
        ...
        # dump detections
        if args.save_result:
            save_path = osp.join('./results/',args.dataset_name,args.model_name+'_'+args.filename,'detection_{}_'+str(epoch)+'_{}.json')
...
def test_freq(model, 
    ...
    for samples, targets in data_loader:
        ...
        # post process
        video_duration = torch.FloatTensor([t["video_duration"] for t in targets]).to(device)
        results = postprocessor(outputs, video_duration, args.eval_proposal, calibrate=True, calibrate_type=type)
        res = {target['video_name']: output for target, output in zip(targets, results)}
        # res_dict.update(res)
