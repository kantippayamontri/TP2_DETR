...
parser.add_argument('--lr_linear_proj_names', default=['reference_points', 'sampling_offsets'], type=str, nargs='+') # nargs='+'：引數至少 1 個（1 個或任意多個）
...
parser.add_argument('--target_type', type=str, default="prompt", choices=('none', 'prompt', 'description', 'name', 'combined_description'), help="[none,prompt,description,name]") # NOTE: 'none' means use one-hot target that just for close_set
...
parser.add_argument('--lr_drop', type=int, default=30, help="the step begin to drop lr")
...
parser.add_argument('--separate_LFOLFT', default=False, action='store_true')
parser.add_argument('--use_LFT', default=False, action='store_true')
...
## ActionFormer(mha)
parser.add_argument('--enable_neck', default=False, action='store_true', help='if add ln at the end of fpn outputs')
parser.add_argument('--enc_stem_layers', type=int, default=1, help="Number of encoding stem layers in the transformer")
parser.add_argument('--enc_branch_layers', type=int, default=3, help="Number of encoding branch layers in the transformer")
parser.add_argument('--win_size', type=int, default=9, help="Window size for local self-attention")
parser.add_argument('--downsample_rate', type=int, default=2, help="Downsampling rate for the branch")
## Can computed from  enc_stem_layers + enc_branch_layers
parser.add_argument('--num_feature_levels', default=4, type=int, help='number of feature levels')
## SGP
parser.add_argument('--use_SGP', default=False, action='store_true')
## init
parser.add_argument('--backbone_init_method', default='v1', type=str, choices=('v0', 'v1', 'v2', 'v3'), help="Type of reset_parameter in backbone network")



# Transformer
parser.add_argument('--enc_layers', type=int, default=5, help="Number of encoding layers in the transformer")
parser.add_argument('--dec_layers', type=int, default=5, help="Number of decoding layers in the transformer")
...
parser.add_argument('--num_queries', type=int, default=40, help="Number of query slots")
parser.add_argument('--dec_n_points', default=4, type=int)
parser.add_argument('--enc_n_points', default=4, type=int)
parser.add_argument('--use_SAPM', default=False, action='store_true')
parser.add_argument('--use_SAattn', default=False, action='store_true')
parser.add_argument('--plainFPN', default=False, action='store_true')
parser.add_argument('--enable_edgePunish', default=False, action='store_true')
parser.add_argument('--punish', type=float, default=2, help="Punish amount for salient loss on edges")
parser.add_argument('--enable_softSalient', default=False, action='store_true')
parser.add_argument('--as_calibration', default=False, action='store_true', help="Whether tp use soft salient_gt as calibration(multiple computed salient loss")
parser.add_argument('--use_decouple', default=False, action='store_true')
parser.add_argument('--visualize_decouple', default=False, action='store_true')
parser.add_argument('--use_VideoSA', default=False, action='store_true')
parser.add_argument('--SA_LastOnly', default=False, action='store_true')
parser.add_argument('--use_TETAD', default=False, action='store_true')
parser.add_argument('--use_Gating', default=False, action='store_true')
parser.add_argument('--no_normalizedqkv', default=False, action='store_true')
...
# Loss
parser.add_argument('--salient_upsample_type', type=str, default='nearest', choices=('nearest', 'linear'),help="Recommend 'nearest' for Thumos14/'linear' for ActivityNet v1.3. Mode of how to upsample different layers of pyramid features.")
parser.add_argument('--salient_aggregate_type', type=str, default="max", choices=('max', 'mean', 'mix', 'mix_adaptive'), help='[max,mean,mix,mix_adaptive]')
parser.add_argument('--separate_predict_head', action='store_true', default=False, help="Whether build box_embed/class_embed/actionness_embed for different dec_layer <-> share same embeds")
...
parser.add_argument('--enlarge_aux_loss', action='store_true')
parser.add_argument('--weightDict', type=float, default=-1)

parser.add_argument('--sparsity_loss', action='store_true')
parser.add_argument('--sparsity_loss_coef', type=float, default=1)

parser.add_argument('--actionness_loss', action='store_true')
...
parser.add_argument('--vis_con_loss', action='store_true', default=False)
parser.add_argument('--vis_con_loss_coef', type=float, default=0.01)
parser.add_argument('--vis_con_loss_temp', type=float, default=0.05)

parser.add_argument('--tIoUw_bbox_loss', action='store_true', default=False)

parser.add_argument('--semantic_guided_loss', action='store_true', default=False)
parser.add_argument('--semantic_guided_loss_coef', type=float, default=3)

parser.add_argument('--feedback_loss', action='store_true', default=False)
parser.add_argument('--feedback_loss_coef', type=float, default=5)
# dataset
## basic
...
parser.add_argument('--feature_projected_path', type=str, default="/mnt/Datasets/Thumos14/CLIP_feature_projected")
## feature info
...
parser.add_argument('--enable_refine_freq', action='store_true', default=False)

# PostProcess
parser.add_argument('--enable_freqCalibrate', action='store_true', default=False)
...
parser.add_argument('--inference_entire', action='store_true', help="Whether to test entire video instead of slide window")
...
parser.add_argument('--filename', type=str, default='', help='When the model is trained.(mmdd_HHMM)')
