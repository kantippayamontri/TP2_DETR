...
class LocalMaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        # output projection
        # self.proj = nn.Conv1d(2*self.n_embd, self.n_embd, 1)
        self.proj = nn.Conv1d(self.n_embd, self.n_embd, 1)
        self.out_proj = nn.Linear(2*self.n_embd, self.n_embd, bias=False)
    ...
    def _sliding_chunks_query_key_matmul(
        self, query, key, num_heads, window_overlap, causal=False
    ):
        """
        ...
        if causal:
            pos = torch.arange(2 * window_overlap + 1, device=query.device)
            causal_mask = (pos > window_overlap).view(1, 1, 1, -1)
            diagonal_attention_scores.masked_fill_(causal_mask, float("-inf"))
    ...
    def forward(self, x, mask, use_Gating=False, no_normalizedqkv=False):
        # x: batch size, feature channel, sequence length,
        # mask: batch size, 1, sequence length (bool)
        B, C, T = x.size()
        ...
        x_downsampled_rev = x_downsampled.flip(-1)  # time reversal
        x_combined = torch.cat([x_downsampled, x_downsampled_rev], dim=0)  # [2B, C, T]
        mask_combined = torch.cat([mask_downsampled, mask_downsampled], dim=0)  # [2B, 1, T]
        if no_normalizedqkv:
            q = k = v = x_combined
        else:
            q = self.query_norm(x_combined)
            k = self.key_norm(x_combined)
            v = self.value_norm(x_combined)
        qx_mask = mask_downsampled
        kv_mask = mask_combined
        q = self.query(q)
        k = self.key(k)
        v = self.value(v)
        v_gating = v
        ...
        # (2B, nh * hs, T) -> (2B, nh, T, hs)
        q = q.view(2*B, self.n_head, self.n_channels, -1).transpose(2, 3)
        k = k.view(2*B, self.n_head, self.n_channels, -1).transpose(2, 3)
        v = v.view(2*B, self.n_head, self.n_channels, -1).transpose(2, 3)
        # view as (2B * nh, T, hs)
        q = q.view(2*B * self.n_head, -1, self.n_channels).contiguous()
        k = k.view(2*B * self.n_head, -1, self.n_channels).contiguous()
        v = v.view(2*B * self.n_head, -1, self.n_channels).contiguous()

        # step 3: compute local self-attention with rel pe and masking
        q *= self.scale
        # chunked query key attention -> 2B, T, nh, 2w+1 = window_size
        ...
        # chunked attn value product -> 2B, nh, T, hs
        out = self._sliding_chunks_matmul_attn_probs_value(
            att, v, self.n_head, self.window_overlap)

        # transpose to 2B, nh, hs, T -> 2B, nh*hs, T
        out = out.transpose(2, 3).contiguous().view(2*B, C, -1)
        
        if use_Gating:
            # out = out * F.silu(x_combined)
            out = out * F.silu(v_gating)

        # 將 forward 與 backward 分開
        out_f, out_b = out.chunk(2, dim=0)  # each is [B, nh*hs, T]
        out_b = out_b.flip(-1)  # 時序反轉
        out = torch.cat([out_f, out_b], dim=1) # [B, 2*nh*hs, T]
        out = out.permute(0, 2, 1)  # [B, T, 2*nh*hs]
        out = self.out_proj(out)    # [B, T, nh*hs]
        out = out.permute(0, 2, 1)  # [B, nh*hs, T]
